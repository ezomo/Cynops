        ...
        match self {
            Declarator::Pointer(p) => {
                let inner = p
                    ...
                    .as_ref()
                    .clone()
                    .expect("Pointer must have inner declarator");
                inner.extract_ident()
            }
            Declarator::Direct(di) => di.extract_ident(),
        }
    }
}

impl Declarator {
    fn extract_paramlist(&self) -> Option<ParamList> {
        match self {
            Declarator::Pointer(p) => {
                let inner = p
                    .inner
                    ...
                    .clone()
                    .expect("Pointer must have inner declarator");
                inner.extract_paramlist()
            }
            Declarator::Direct(direct) => direct.extract_paramlist(),
        }
    }
}
...
impl DirectDeclarator {
    fn extract_paramlist(&self) -> Option<ParamList> {
        match self {
            DirectDeclarator::Func(f) => f.params.clone(),
            DirectDeclarator::Array(a) => a.base.extract_paramlist(),
            DirectDeclarator::Paren(p) => p.extract_paramlist(),
...
impl DirectDeclarator {
    fn extract_ident(&self) -> Ident {
        match self {
            DirectDeclarator::Array(a) => a.base.extract_ident(),
            DirectDeclarator::Func(f) => f.base.extract_ident(),
            DirectDeclarator::Paren(p) => p.extract_ident(),
            DirectDeclarator::Ident(i) => i.clone(), // cloneする必要がある場合
        }
    }
}
...
impl ParamList {
    pub fn to_vec(&self) -> Vec<Param> {
        match self {
            ParamList::Void => vec![],
            ParamList::Params(params) => params.clone(),
        }
    }
}
    ...
    match stmt {
        ...
        Stmt::DeclStmt(decl) => match decl {
            DeclStmt::Typed(typed) => {
                ...
                let ptr = cgs
                    .variables
                    .entry(typed.declarators[0].declarator.extract_ident())
                    .or_insert_with(|| {
...
fn gen_function(function: FunctionDef, cgs: &mut CodeGenStatus) -> String {
    let name = function.sig.declarator.extract_ident();
    let args: Vec<String> = function
        ...
        .unwrap_or_default(); // None の場合は Vec::new() を返す
...
// 互換性のための関数（古いインターフェース）
pub fn generate(top_level: TopLevel, cgs: &mut CodeGenStatus) -> String {
    gen_top_level(top_level, cgs)
}
