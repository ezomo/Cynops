    ...
    fn visualize_with_context(&self, indent: usize, is_last: bool, prefix: &[bool]) {
        match self {
            ...
            Expr::Postfix(postfix) => {
                postfix.visualize_with_context(indent, is_last, prefix);
            }
            Expr::Call(call) => {
...
impl OneLine for Postfix {
    fn oneline(&self) -> String {
        format!("{}{}", self.expr.oneline(), self.op.to_string())
    }
}
...
impl OneLine for Subscript {
    fn oneline(&self) -> String {
        format!("{}[{}]", self.name.oneline(), self.index.oneline())
    }
}
...
impl OneLine for Sizeof {
    fn oneline(&self) -> String {
        match self {
            ...
            Sizeof::Expr(expr) => format!("sizeof({})", expr.oneline()),
        }
    }
}
...
impl OneLine for Ident {
    fn oneline(&self) -> String {
        self.name.clone()
    }
}

// Main implementation for Expr that delegates to individual types
impl OneLine for Expr {
    fn oneline(&self) -> String {
        match self {
            Expr::NumInt(n) => n.to_string(),
            Expr::NumFloat(n) => n.to_string(),
            Expr::Char(c) => format!("'{}'", c),
            Expr::String(chars) => {
                let s: String = chars.iter().collect();
                format!("\"{}\"", s)
            }
            Expr::Ident(ident) => ident.oneline(),
            Expr::Binary(binary) => binary.oneline(),
            Expr::Unary(unary) => unary.oneline(),
            Expr::Postfix(postfix) => postfix.oneline(),
            Expr::Call(call) => call.oneline(),
            Expr::Assign(assign) => assign.oneline(),
            Expr::Ternary(ternary) => ternary.oneline(),
            Expr::Subscript(subscript) => subscript.oneline(),
            Expr::MemberAccess(member_access) => member_access.oneline(),
            Expr::Sizeof(sizeof) => sizeof.oneline(),
            Expr::Cast(cast) => cast.oneline(),
            Expr::Comma(comma) => comma.oneline(),
        }
    }
}
